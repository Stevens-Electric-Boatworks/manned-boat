...
ROSOutLogger::ROSOutLogger(const std::shared_ptr<IDataReceiver<type>> &data, const std::shared_ptr<IDataTransmitter>& transmitter, bool replay_mode)
...
void ROSOutLogger::on_data(const type data) const {
    this->data_transmitter->send_log(LogData{get_time_from_msg(data.stamp), data.msg, data.file,
