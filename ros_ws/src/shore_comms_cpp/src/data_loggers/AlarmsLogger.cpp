...
AlarmsLogger::AlarmsLogger(const std::shared_ptr<IDataReceiver<type>> &data, const std::shared_ptr<IDataTransmitter>& transmitter, bool replay_mode)
...
void AlarmsLogger::on_data(const type data) const {
    auto const alarm = Alarm{
      data.error_code, get_time_from_msg(data.timestamp)
    };
